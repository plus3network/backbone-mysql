...
ORM.model.create = function (model, options) {
  ...
  client.query(query, values, function (err, results) {
    if (err && options.error) return options.error.call(null, err, options);
    data.id = results.insertId; 
    model.set(data);
    if (options.success) options.success.call(null, data, options);
...
ORM.model.read = function (model, options) {
  ...
  client.query(query, [model.id], function (err, results) {
    if (err && options.error) return options.error.call(null, err, options);
    if (results.length === 0 && options.error) return options.error.call(null, new Error('Resource Not Found'), null, options);
    var data = results[0];
    if (options.success) options.success.call(null, data, options);
  });
};
